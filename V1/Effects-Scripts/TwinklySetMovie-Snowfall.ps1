...
# Define the mapping of IP addresses to specific movies
$ipToMovieMap = @{
    "192.168.68.176" = "Snowfall Sparkl"
    "192.168.68.175" = "Snowfall Sparkl"
    "192.168.68.158" = "Snowfall Sparkl"
    "192.168.68.143" = "Snowfall Sparkl"
}

foreach ($ip in $ipToMovieMap.Keys) {
    Write-Output "Processing IP: $ip"

    # Get the movie name for this IP
    $movieName = $ipToMovieMap[$ip]
    ...
    if ($token) {
        ...
        if ($isVerified) {
            ...
            if ($movieMode) {
                ...
                if ($movies) {
                    # Find the movie specified in the mapping
                    $movieToSet = $movies | Where-Object { $_.name -eq $movieName }
                    if ($movieToSet) {
                        # Set the movie as the current movie
                        $setMovie = Set-CurrentMovie -IpAddress $ip -AuthToken $token -UniqueId $movieToSet.unique_id
                        if ($setMovie) {
                            Write-Output "Successfully set the current movie to '$movieName' for IP $ip."
                        } else {
                            Write-Warning "Failed to set the movie '$movieName' for IP $ip."
                        }
                    } else {
                        Write-Warning "Movie '$movieName' not found for IP $ip."
                    }
                } else {
